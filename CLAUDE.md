...
- **Scissors:** `{ edge: 'top'|'bottom'|'left'|'right', pos: number, cutting: boolean, cutStart: {x,y}, cutCurrent: {x,y}, cutEdge: string, cutPos: number }` — `cutEdge`/`cutPos` record which edge and position the cut originated from (used for rectangle splitting)
