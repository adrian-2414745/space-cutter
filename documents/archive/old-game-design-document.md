...
**Core Loop:** The player shrinks a polygon playing field using L-shaped cuts (corner nibbles) or straight cuts (full splits) while avoiding bouncing balls. Ball count is governed by a density formula that increases crowding as the field shrinks, creating escalating difficulty.
...
- Ball count is derived from the current polygon area using a density formula. Initial count is seeded from the starting area.
- After each successful cut, balls in the removed portion are discarded and the remaining count is reconciled to the density target.
...
1. **Phase 1 (Inward Travel):**
   ...
   - A faint preview line shows the predicted straight cut path.
...
4. **Failure:** If any bouncing ball intersects the cut line while the cut is in progress (L-cut or straight cut), the entire cut is cancelled.
 The line disappears and the scissors resets to its starting position on the perimeter.
...
### 5.5 Ball Reconciliation
After every successful cut (L-cut or straight cut) and at game start, the ball count is reconciled to a density-based target:

1. **Containment filter:** Balls inside the removed portion are discarded.
2. **Compute target:**
   ```
   areaPercent  = (currentArea / originalArea) * 100
   progressFactor = 1 + K × (1 - areaPercent / 100)
   effectiveDensity = BALL_DENSITY_PX2 / progressFactor
   targetBalls = max(MIN_BALLS, floor(currentArea / effectiveDensity))
   ```
3. If live balls < target — spawn the difference at random positions inside the polygon.
4. If live balls > target — remove the excess, chosen at random.

The progressive ramp factor (K) causes effective density to tighten as the polygon shrinks, so ball crowding increases over the course of a game rather than staying flat. New balls are assigned a random direction and the same base speed as existing balls.
...
1. **Progressive ball density** — the density ramp (K) causes effective px² per ball to decrease as the polygon shrinks. At K=2 and 15% remaining area, ball crowding is ~2.7× tighter than at game start.
...
3. **Constant ball speed** — ball speed does not change, but faster-moving balls in a tighter space produce more frequent cut-line collisions. Speed is configurable before starting.
...
| Ball density | 33,000 px² per ball | Base area allocated per ball (before ramp) |
| Min balls | 2 | Floor — ball count never drops below this |
| Density ramp (K) | 2 | Progressive crowding factor; higher K = steeper difficulty curve |
| Ball speed | 150 (120 mobile) | Base movement speed of balls |
| Ball radius | 6 | Size of the bouncing balls |
| Scissors border speed | 400 | Speed of scissors moving along the perimeter |
