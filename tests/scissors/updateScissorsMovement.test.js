...
describe('updateScissorsMovement', () => {
  ...
  it('1: no input (left=false, right=false) — scissors do not move', () => {
    ...
    const result = updateScissorsMovement(sc, testPoly, 0.1, config, { left: false, right: false });
  ...
  it('2: right input — pos increases by speed*dt (200*0.1=20)', () => {
    const sc = makeScissors(0, 100);
    const result = updateScissorsMovement(sc, testPoly, 0.1, config, { left: false, right: true });
  ...
  it('3: left input — pos decreases by speed*dt (200*0.1=20)', () => {
    const sc = makeScissors(0, 100);
    const result = updateScissorsMovement(sc, testPoly, 0.1, config, { left: true, right: false });
  ...
  it('4: both inputs — net delta is 0, scissors do not move', () => {
    const sc = makeScissors(0, 100);
    const result = updateScissorsMovement(sc, testPoly, 0.1, config, { left: true, right: true });
    expect(result.edgeIndex).toBe(0);
    expect(result.pos).toBe(100);
  });
  ...
  it('7: wrap overflow clamped to next edge\'s length (large overshoot rightward)', () => {
    ...
    const result = updateScissorsMovement(sc, testPoly, 10, config, { left: false, right: true });
    expect(result.edgeIndex).toBe(1);
    expect(result.pos).toBe(400);
  });
  ...
  it('8: wrapped pos clamped to 0 if still negative (large leftward overshoot)', () => {
    ...
    const sc = makeScissors(1, 5);
    const result = updateScissorsMovement(sc, testPoly, 10, config, { left: true, right: false });
    expect(result.edgeIndex).toBe(0);
  ...
  it('9: right snap — within cornerSnapDistance of edge end, moving right → snap to len', () => {
    ...
    const result = updateScissorsMovement(sc, testPoly, 0.001, config, { left: false, right: true });
    expect(result.edgeIndex).toBe(0);
    expect(result.pos).toBe(600);
  });
  ...
  it('10: left snap — within cornerSnapDistance of edge start, moving left → snap to 0', () => {
    ...
    const result = updateScissorsMovement(sc, testPoly, 0.001, config, { left: true, right: false });
    expect(result.edgeIndex).toBe(0);
    expect(result.pos).toBe(0);
  });
  ...
  it('11: snap only fires when moving toward corner (not away)', () => {
    ...
    const result = updateScissorsMovement(sc, testPoly, 0.1, config, { left: false, right: true });
    expect(result.edgeIndex).toBe(0);
    expect(result.pos).toBe(23);
  });

  it('12: dt=0 — no movement even with right input', () => {
    const sc = makeScissors(0, 100);
    const result = updateScissorsMovement(sc, testPoly, 0, config, { left: false, right: true });
    expect(result.edgeIndex).toBe(0);
    expect(result.pos).toBe(100);
  });
  ...
  it('14: does not mutate input scissors', () => {
    const sc = makeScissors(0, 100);
