...
describe('updateScissorsMovementTouch — basic movement', () => {
  it('1: positive dx — pos increases', () => {
    ...
    const result = updateScissorsMovementTouch(scissors, testPoly, config, 50);
  ...
  it('2: negative dx — pos decreases', () => {
    const scissors = makeScissors(0, 100);
    const result = updateScissorsMovementTouch(scissors, testPoly, config, -30);
    expect(result.pos).toBe(70);
    expect(result.edgeIndex).toBe(0);
  });
  ...
  it('3: touchSensitivity scales movement', () => {
    ...
    const result = updateScissorsMovementTouch(scissors, testPoly, cfg, 10);
...
describe('updateScissorsMovementTouch — dead zone', () => {
  it('4: |dx * sensitivity| < 0.01 — no movement', () => {
    const scissors = makeScissors(0, 100);
    const result = updateScissorsMovementTouch(scissors, testPoly, config, 0.005);
  ...
  it('5: dx=0 — no movement', () => {
    const scissors = makeScissors(0, 100);
    const result = updateScissorsMovementTouch(scissors, testPoly, config, 0);
    expect(result.pos).toBe(100);
    expect(result.edgeIndex).toBe(0);
  });
});
...
describe('updateScissorsMovementTouch — edge wrapping', () => {
  it('6: positive dx wraps to next edge — pos set to 0 (not carry-over)', () => {
    ...
    const result = updateScissorsMovementTouch(scissors, testPoly, config, 20);
  ...
  it('10: wraps from edge 3 to edge 0 with positive dx', () => {
    ...
    const result = updateScissorsMovementTouch(scissors, testPoly, config, 10);
    expect(result.edgeIndex).toBe(0);
    expect(result.pos).toBe(0);
  });
});
...
describe('updateScissorsMovementTouch — corner snapping', () => {
  it('8: snap to end corner, positive delta, pos near edge end', () => {
    ...
    const result = updateScissorsMovementTouch(scissors, testPoly, config, 1);
    expect(result.edgeIndex).toBe(0);
    expect(result.pos).toBe(600);
  });
...
describe('updateScissorsMovementTouch — does not mutate input', () => {
  it('11: does not mutate input scissors', () => {
    const scissors = makeScissors(0, 100);
