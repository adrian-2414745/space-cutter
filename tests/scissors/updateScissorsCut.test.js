...
describe('updateScissorsCut — phase 1', () => {
    it('1: phase 1, moves cutCurrent in "down" direction — y increases by speed*dt (150*0.1=15)', () => {
        ...
        const result = updateScissorsCut(scissors, null, 0.1, config);
        expect(result.cutCurrent.y).toBeCloseTo(15, 10);
        expect(result.cutCurrent.x).toBeCloseTo(300, 10);
    });
    ...
    it('2: phase 1, "up" direction decreases y — y goes from 200 to 185', () => {
        ...
        const result = updateScissorsCut(scissors, null, 0.1, config);
        expect(result.cutCurrent.y).toBeCloseTo(185, 10);
        expect(result.cutCurrent.x).toBeCloseTo(300, 10);
    });
    ...
    it('3: phase 1, "right" direction increases x — x goes from 100 to 115', () => {
        ...
        const result = updateScissorsCut(scissors, null, 0.1, config);
        expect(result.cutCurrent.x).toBeCloseTo(115, 10);
        expect(result.cutCurrent.y).toBeCloseTo(200, 10);
    });
    ...
    it('4: phase 1, "left" direction decreases x — x goes from 300 to 285', () => {
        ...
        const result = updateScissorsCut(scissors, null, 0.1, config);
        expect(result.cutCurrent.x).toBeCloseTo(285, 10);
        expect(result.cutCurrent.y).toBeCloseTo(200, 10);
    });
    ...
    it('5: phase 1, dt=0 produces no movement — cutCurrent unchanged', () => {
        ...
        const result = updateScissorsCut(scissors, null, 0, config);
        expect(result.cutCurrent.x).toBe(300);
        expect(result.cutCurrent.y).toBe(100);
    });
});
...
describe('updateScissorsCut — phase 2', () => {
    ...
    it('7: phase 2, clamps to target when remaining <= speed (remaining=5, speed=15)', () => {
        ...
        const result = updateScissorsCut(scissors, null, 0.1, config);
        expect(result.cutCurrent.x).toBe(300);
        expect(result.cutCurrent.y).toBe(200);
    });
    ...
    it('8: phase 2, does not overshoot target — cutCurrent matches cutTarget exactly after clamp', () => {
        ...
        const result = updateScissorsCut(scissors, null, 0.1, config);
        expect(result.cutCurrent.x).toBe(result.cutTarget.x);
        expect(result.cutCurrent.y).toBe(result.cutTarget.y);
    });
});
...
describe('updateScissorsCut — does not mutate input', () => {
    it('10: does not mutate input scissors in phase 1', () => {
        const scissors = makeScissors({ cutPhase: 1, cutDirection: 'down', cutCurrent: { x: 300, y: 0 } });
        const origX = scissors.cutCurrent.x;
        const origY = scissors.cutCurrent.y;
        updateScissorsCut(scissors, null, 0.1, config);
        expect(scissors.cutCurrent.x).toBe(origX);
        expect(scissors.cutCurrent.y).toBe(origY);
    });

    it('11: does not mutate input scissors in phase 2', () => {
        const scissors = makeScissors({
            cutPhase: 2,
            cutTurnDirection: 'right',
            cutCurrent: { x: 100, y: 200 },
